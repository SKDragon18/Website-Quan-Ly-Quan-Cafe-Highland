...
<form id="myForm" method="POST">
  <input type="hidden" name="MANL" value="" />
  <input type="hidden" name="SLT" value="" />
  <input type="hidden" name="MAHD" value="" />
  ...
  <input type="hidden" name="KMAI" value="" />
</form>
  ...
  <div id="product-container">
    <c:forEach var="sanpham" items="${listSP}">
      <div class="product" data-maloai="${sanpham.LOAISP.MALOAI}">
        <img src="${root}/resources/img/sanpham/${sanpham.HINHANH}" alt="${sanpham.TEN}" />
        <h3>${sanpham.TEN}</h3>
        <div class="sizes">
        ...
        <div class="action-buttons" data-test="${sanpham.TEN }">
			<c:set var="buttonCreated" value="false" scope="request" />
			<c:forEach var="ctsp" items="${listCTSP}">
			  <c:if test="${!buttonCreated && sanpham.MASP == ctsp.getSANPHAM().getMASP()}">
			    <c:choose>
			      <c:when test="${ctsp.getSIZE().getMASIZE() == 'K'}">
			        <c:set var="buttonCreated" value="true" scope="request" />
			        <button class="add-button" data-product-id="${sanpham.MASP}" data-product-name="${sanpham.TEN}" data-product-size="K" data-product-quantity="1" data-product-price="${ctsp.GIAHIENTHOI}" data-product-max="1000" data-product-ct="${ctsp.getCONGTHUC().getMACT()}">Add</button>
			      </c:when>
			      <c:when test="${ctsp.getSIZE().getMASIZE() == 'L'}">
			        <c:set var="buttonCreated" value="true" scope="request" />
			        <button class="add-button" data-product-id="${sanpham.MASP}" data-product-name="${sanpham.TEN}" data-product-size="L" data-product-quantity="1" data-product-price="${ctsp.GIAHIENTHOI}" data-product-max="1000" data-product-ct="${ctsp.getCONGTHUC().getMACT()}">Add</button>
			      </c:when>
			      <c:when test="${ctsp.getSIZE().getMASIZE() == 'M'}">
			        <c:set var="buttonCreated" value="true" scope="request" />
			        <button class="add-button" data-product-id="${sanpham.MASP}" data-product-name="${sanpham.TEN}" data-product-size="M" data-product-quantity="1" data-product-price="${ctsp.GIAHIENTHOI}" data-product-max="1000" data-product-ct="${ctsp.getCONGTHUC().getMACT()}">Add</button>
			      </c:when>
			      <c:when test="${ctsp.getSIZE().getMASIZE() == 'S'}">
			        <c:set var="buttonCreated" value="true" scope="request" />
			        <button class="add-button" data-product-id="${sanpham.MASP}" data-product-name="${sanpham.TEN}" data-product-size="S" data-product-quantity="1" data-product-price="${ctsp.GIAHIENTHOI}" data-product-max="1000" data-product-ct="${ctsp.getCONGTHUC().getMACT()}">Add</button>
			      </c:when>
			    </c:choose>
			  </c:if>
			</c:forEach>
...
function sendCTSP() {
    ...
    for (let i = 0; i < productDetails.length; i++) {
        ...
        if ((isProductChecked) === "true") {
            ...
            form.elements.MASP.value += masp + ',';
            form.elements.MASIZE.value += size + ',';
            form.elements.SOLUONG.value += quantity + ',';
        }
    }
}

function sendDataToServer() {
	form.elements.MANV.value = nvID;
	form.elements.MAHD.value = nextID;
	form.elements.TONGTIEN.value = total;
	form.elements.KMAI.value = makm;
}

function sendNL() {
    for (let i = 0; i < nlList.length; i++) {
    	form.elements.MANL.value += nlList[i].maNL + ',';
    	form.elements.SLT.value += nlList[i].slt + ',';
    }
}
